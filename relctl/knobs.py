...
KNOBS = [
    ...
    Knob("n_aug", "model", "train", "int", 2, valid=(0, None), fw_scope="looc",
         coupling="reporting only; the actual spaces follow len(looc_augs)",
         doc="Number of augmentation-specific LooC spaces (kept in sync with looc_augs)."),
    Knob("looc_augs", "model", "train", "list_str", ["rotation", "color"], fw_scope="looc",
         coupling="each group adds a projection head, a queue and a key view per step",
         doc="LooC augmentation groups, one embedding space each: rotation, color, "
             "hflip, grayscale, blur, crop."),
    Knob("full_multiview", "model", "train", "bool", True, fw_scope="looc",
         coupling="false reduces LooC exactly to MoCo -- not a LooC baseline",
         doc="Enable LooC's per-augmentation embedding spaces (the method's contribution)."),
...
FRAMEWORK_KNOBS = {
    ...
    "looc": ["dim", "K", "m", "T", "n_aug", "looc_augs", "full_multiview"],
